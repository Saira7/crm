...
export default function LeadDetailModal({
  ...
  token,           // auth token (apiFetch will use)
  ...
  useEffect(() => {
    ...
    setLocal({
      ...
      tagsText: Array.isArray(lead.tags) ? lead.tags.join(', ') : (lead.tags || '').toString(),
      ...
      saleDate_local: lead.saleDate ? new Date(lead.saleDate).toISOString().split('T')[0] : '',
      dob1_local: lead.dob1 ? new Date(lead.dob1).toISOString().split('T')[0] : '',
      dob2_local: lead.dob2 ? new Date(lead.dob2).toISOString().split('T')[0] : ''
    });
  ...
  // FIXED: users available to assign depending on role
  ...
  const buildPatch = () => {
    ...
    for (const f of changedFields) {
      switch (f) {
        ...
        case 'tagsText':
          patch.tags = local.tagsText ? local.tagsText.split(',').map(t => t.trim()).filter(Boolean) : [];
          break;
        case 'dueDate_date':
          patch.dueDate = local.dueDate_date ? local.dueDate_date.toISOString() : null;
          break;
        case 'saleDate_local':
          patch.saleDate = local.saleDate_local ? new Date(local.saleDate_local).toISOString() : null;
          break;
        case 'dob1_local':
          patch.dob1 = local.dob1_local ? new Date(local.dob1_local).toISOString() : null;
          break;
        case 'dob2_local':
          patch.dob2 = local.dob2_local ? new Date(local.dob2_local).toISOString() : null;
          break;
        default:
  ...
  // Add a status comment (lead/callback/sale) - stores comment and comment date
  const handleAddStatusComment = async (statusKey, commentText) => {
    ...
    try {
      ...
      setLocal(prev => ({ ...prev, [commentField]: commentText, [dateField]: payload[dateField] }));
    } catch (err) {
  ...
  const handleAssign = async (assignedToId) => {
    const payload = {
      ...
      teamName: assignedToId ? (assignableUsers.find(u => u.id === parseInt(assignedToId,10))?.team?.name ?? assignableUsers.find(u => u.id === parseInt(assignedToId,10))?.team ?? null) : null
    };
    setSaving(true);
    try {
      await onUpdate(local.id, payload);
  ...
  const InputRow = ({ label, name, type = 'text', placeholder = '', className = '', rows = 1 }) => {
    if (rows === 1) {
      return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl w-full max-w-6xl max-h-[92vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-start justify-between gap-4 z-10">
          <div>
            <h3 className="text-xl font-bold">{local.companyName || 'Lead'}</h3>
            <div className="text-sm text-gray-500">{local.companyType && `${local.companyType} • `}{local.businessNature}</div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Edit2 className="w-4 h-4" /> {editing ? 'Editing' : 'Edit'}
            </button>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <InputRow label="Company Name" name="companyName" />
            ...
            <InputRow label="Registration Number" name="registrationNumber" />
            <InputRow label="Number of Directors" name="numberOfDirectors" type="number" />
          ...
          {/* Directors / Owners */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <InputRow label="Director/Owner 1 - Name" name="directorOwnerName1" />
            <InputRow label="Position 1" name="position1" />
            <InputRow label="Nationality 1" name="nationality1" />

            <InputRow label="Director/Owner 1 - Home Address" name="homeAddress1" rows={2} />

            <InputRow label="DOB 1" name="dob1_local" type="date" />
            <InputRow label="Director/Owner 2 - Name" name="directorOwnerName2" />
            <InputRow label="Position 2" name="position2" />

            <InputRow label="DOB 2" name="dob2_local" type="date" />
            <InputRow label="Director/Owner 2 - Home Address" name="homeAddress2" rows={2} />
            <InputRow label="Nationality 2" name="nationality2" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <InputRow label="Existing Company" name="existingCompany" />
            <InputRow label="Remaining Terms" name="remainingTerms" />
            <InputRow label="Already Paying Charges" name="alreadyPayingCharges" />
            <InputRow label="Details" name="details" rows={2} />

            <InputRow label="Total Turnover" name="totalTurnover" type="number" />
            <InputRow label="Card Turnover" name="cardTurnover" type="number" />
            <InputRow label="Avg Transaction" name="avgTransaction" type="number" />
            <InputRow label="Min Transaction" name="minTransaction" type="number" />
            <InputRow label="Max Transaction" name="maxTransaction" type="number" />
            <InputRow label="Monthly Line Rent" name="monthlyLineRent" type="number" />
            <InputRow label="Debit Percentage" name="debitPercentage" type="number" />
            <InputRow label="Credit Percentage" name="creditPercentage" type="number" />
            <InputRow label="Authorization Fee" name="authorizationFee" type="number" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <InputRow label="Account Number" name="accountNumber" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-end">
            ...
            <div>
              ...
              />
              <div className="text-xs text-gray-500 mt-1">Comma separated tags.</div>
            </div>

            <div className="lg:col-span-3">
              <label className="block text-xs text-gray-600 mb-1">Lead Comment (initial/summary)</label>
              <textarea
                rows={3}
                className="w-full px-3 py-2 border rounded"
                value={local.leadComment ?? ''}
                onChange={(e) => onChange('leadComment', e.target.value)}
                disabled={!editing}
              />
              <div className="flex items-center justify-between text-xs text-gray-500 mt-1">
                <div>Last comment date: {local.leadCommentDate ? new Date(local.leadCommentDate).toLocaleString() : '—'}</div>
                <div className="text-right">You can also add comment entries below</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-end">
            <div>
              ...
              <select
                className="w-full px-3 py-2 border rounded"
                value={local.assignedToId || user.id} // Default to current user if null
                onChange={(e) => {
              ...
              >
                {assignableUsers.map(u => (
                  <option key={u.id} value={u.id}>
                    {u.name} 
                    {u.team ? ` — ${typeof u.team === 'string' ? u.team : u.team.name}` : ''}
                    {typeof u.role === 'string' ? ` (${u.role})` : u.role?.name ? ` (${u.role.name})` : ''}
                  </option>
                ))}
              </select>
          ...
          {/* Status comment adders (lead/callback/sale) */}
          <div className="space-y-4">
            <h4 className="text-sm font-semibold">Add Status Comments</h4>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <StatusCommentBox
                ...
                onAdd={(text) => handleAddStatusComment('callback', text)}
                disabled={!editing}
              />
              <StatusCommentBox
                title="Sale Comment"
...
function StatusCommentBox({ title, existingComment, existingDate, onAdd, disabled }) {
  const [txt, setTxt] = useState('');
  useEffect(() => { setTxt(''); }, [existingComment]);
  return (
    <div className="p-4 border rounded">
      <div className="flex items-center justify-between mb-2">
        <div className="text-sm font-medium">{title}</div>
        <div className="text-xs text-gray-500">{existingDate ? new Date(existingDate).toLocaleString() : 'No date'}</div>
      </div>

      <div className="text-sm text-gray-700 mb-3">{existingComment || <span className="text-gray-400">No comment</span>}</div>
