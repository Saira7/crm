...
export default function AdminDashboard() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await apiFetch(`/admin/overview${qs ? `?${qs}` : ''}`, token);
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            High-level overview of leads, sales, teams and agents across the system.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500 uppercase font-semibold">Callbacks</p>
                ...
                {byTeam.map((row) => {
                  const conv =
                    row.totalLeads > 0
                      ...
                      : '0.0';
                  return (
                    <tr key={row.teamName || 'none'} className="border-b last:border-b-0">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        ...
        {byAgent.length === 0 ? (
          <p className="text-xs text-gray-500">No agent activity for this range.</p>
        ) : (
