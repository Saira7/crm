...
export default function TeamPage() {
  ...
  const getUserLeadCount = (uid) =>
    leads.filter((l) => l.assignedToId === uid).length;
  const getUserRole = (u) =>
    typeof u.role === 'string' ? u.role : u.role?.name;
  const getUserTeam = (u) =>
    typeof u.team === 'string' ? u.team : u.team?.name;
  const getUserTeamId = (u) =>
    u.teamId || (u.team && u.team.id) || null;

  // teams this TL leads, from /teams.leads
  const leadTeamIds = useMemo(() => {
    if (!isTeamLeadRole) return [];
    return teams
      .filter((t) =>
        (t.leads || []).some((lt) => lt.userId === userId)
      )
      .map((t) => t.id);
  }, [teams, isTeamLeadRole, userId]);

  const filteredUsers = useMemo(() => {
    return users.filter((u) => {
      if (isAdmin) return true;

      const memberTeamId = getUserTeamId(u);
      const memberTeamName = getUserTeam(u);

      if (isTeamLeadRole) {
        if (leadTeamIds.length > 0) {
          return memberTeamId && leadTeamIds.includes(memberTeamId);
        }
        return memberTeamName === userTeam;
      }

      return memberTeamName === userTeam;
    });
  }, [users, isAdmin, isTeamLeadRole, leadTeamIds, userTeam]);

  const canEditUser = (member) => {
    if (!userRole) return false;
    if (member?.id === userId) return false;

    const roleNormalized = normRole;
    if (roleNormalized === 'admin') return true;

    if (
      roleNormalized === 'team_lead' ||
      roleNormalized === 'team lead' ||
      roleNormalized === 'team-lead'
    ) {
      const memberTeamId = getUserTeamId(member);
      if (leadTeamIds.length > 0) {
        return memberTeamId && leadTeamIds.includes(memberTeamId);
      }
  ...
  const saveEdit = async () => {
    ...
    if ((isAdmin || isTeamLead) && editTeam) {
      const targetTeam = teams.find((t) => t.name === editTeam);
      if (!targetTeam) {
        return setError('Invalid team selected');
      }
      if (isTeamLead && leadTeamIds.length > 0 &&
          !leadTeamIds.includes(targetTeam.id)) {
        return setError(
          'Team Leads can only assign members to teams they lead'
        );
      }
    }
    ...
    try {
      ...
      if (isAdmin || isTeamLead) {
        ...
        if (
          isTeamLead &&
          foundRole &&
          foundRole.name.toLowerCase() === 'admin'
        ) {
  ...
  const openAddModal = () => {
    const defaultRole =
      roles.find((r) => r.name.toLowerCase().includes('sales'))?.name ||
      roles[0]?.name ||
      '';
    setAdding(true);
    setAddName('');
  ...
  const saveAdd = async () => {
    ...
    if (isTeamLead) {
      // For now, still default TL-created users to their own home team
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-700">
                    {getUserTeam(member) || 'No team'}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Briefcase className="w-4 h-4 text-gray-400 flex-shrink-0" />
                  <span className="text-gray-700">
                    {getUserLeadCount(member.id)} active leads
                  </span>
                </div>
              </div>
            </div>
